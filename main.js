...
const state = {
    ...
    ring: [], // pin positions
    path: [],
    target: null, // Float32Array grayscale (1=dark) OR null when geen afbeelding
...
function preprocessToTarget() {
    ...
    if (!state.img) {
        ...
        return null; // <<< BELANGRIJK: geen target zonder afbeelding
    }
    // cover fit
    ...
    // first pass: grayscale + tone mapping (correcte dubbele lus)
    for (let i = 0, y = 0; y < d; y++) {
        for (let x = 0; x < d; x++, i++) {
            const k = i * 4;
            // luminance
    ...
    if (eg > 0) {
        ...
        for (let i2 = 0; i2 < out.length; i2++) out[i2] = clamp(out[i2] * (1) + sob[i2] * a * 0.7, 0, 1);
    }
...
function renderTarget() {
    ...
    state.target = target; // kan null zijn
    resetCurrent();
    buildRing();
    drawBoard();
    setStatus(target ? 'Doelbeeld klaar.' : 'Upload eerst een afbeelding.');
}
...
function drawLineToCurrent(aIdx, bIdx) {
    ...
    const a = state.ring[aIdx], b = state.ring[bIdx];
    artCtx.save();
    artCtx.globalAlpha = 1; artCtx.strokeStyle = state.lineColor; artCtx.lineWidth = state.thickness; artCtx.lineCap = 'round'; artCtx.lineJoin = 'round';
    artCtx.beginPath(); artCtx.moveTo(a.x, a.y); artCtx.lineTo(b.x, b.y); artCtx.stroke();
    artCtx.restore();
}
...
function stepOnce() {
    ...
    if (!state.target) { setStatus('Upload eerst een afbeelding.'); return false; }
...
function exportSVG() {
    ...
    for (let i = 0; i < state.ring.length; i++) {
        ...
        svg += `<circle cx="${p.x.toFixed(2)}" cy="${p.y.toFixed(2)}" r="2" fill="#94a3b8"/>`;
    }
    // lines
    for (let i = 1; i < state.path.length; i++) {
...
(function init() {
    ...
    resizeCanvases();
    buildRing();
    renderTarget(); // toont placeholder en vereist afbeelding voor stappen
    drawBoard();
})();